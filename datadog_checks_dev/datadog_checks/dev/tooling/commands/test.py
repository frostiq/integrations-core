...
def check_requirements(check, changed_files):
    """
    ...
    if changed_files and not (req_in in changed_files and req_txt in changed_files):
        echo_info('Skip, requirements did not change.')
        return
...
@click.command(
    ...
    short_help='Run the tests for Agent-based checks'
)
@click.argument('checks', nargs=-1)
@click.option(
    '--changed-only', '-c',
    is_flag=True,
    help='Only test checks that were changed compared to the master branch'
)
@click.option('--bench', '-b', is_flag=True, help='Run only benchmarks')
@click.option('--every', is_flag=True, help='Run every kind of test')
def test(checks, changed_only, bench, every):
    """Run the tests for Agent-based checks."""
    if not checks:
        changed_only = True
        checks = get_testable_checks()
    else:
        testable_checks = set(get_testable_checks())
        checks = [c for c in checks if c in testable_checks]
    ...
    if changed_only:
        ...
        checks = set(checks) & changed_checks
    ...
    for check in checks:
        ...
        if not bench:
            ...
            check_requirements(check, changed_files)
