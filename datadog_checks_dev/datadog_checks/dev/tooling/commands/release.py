...
from ...utils import chdir, env_vars, read_file_lines, run_command, write_file, write_file_lines
...
@click.group(
    ...
    short_help='Manages the release of checks'
)
def release():
    pass
...
@release.command(
    context_settings=CONTEXT_SETTINGS,
    short_help='Tag the git repo with the current release number for a check'
)
@click.argument('check')
...
def make(ctx, check, version):
    ...
    if p_version <= p_current:
        abort('Current version is {}, cannot bump to {}'.format(p_current, p_version))

    # update the version number
    echo_info('Current version of check {}: {}, bumping to: {}'.format(check, p_current, p_version))
    update_version_module(check, cur_version, version)

    # update the CHANGELOG
    echo_waiting('Updating the changelog...')
    ctx.invoke(changelog, check=check, version=version, dry_run=False)
...
@click.option('--dry_run', '-n', is_flag=True)
@click.pass_context
def changelog(ctx, check, version, dry_run):
...
@release.command(
    context_settings=CONTEXT_SETTINGS,
    short_help='Upload a check to PyPI'
)
@click.argument('check')
@click.option('--dry_run', '-n', is_flag=True)
@click.pass_context
def upload(ctx, check, dry_run):
    ...
    echo_waiting('Building and publishing {} on PyPI...'.format(check))

    with chdir(os.path.join(get_root(), check)), env_vars(auth_env_vars):
        result = run_command('python setup.py bdist_wheel', capture='out')
        if result.code != 0:
...
@release.command(
    context_settings=CONTEXT_SETTINGS,
    short_help='Update the `requirements-agent-release.txt` file'
)
def freeze():
    ...
    write_file_lines(os.path.join(get_root(), AGENT_REQ_FILE), lines)
